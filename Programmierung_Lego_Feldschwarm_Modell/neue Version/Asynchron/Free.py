...
def getDistance(s: socket):
    ...
    if (dataDistance.startswith(bytes('OK', 'UTF-8'))
            and dataDistance.endswith(bytes('\r\n', 'UTF-8'))):
        valuedistance2 = dataDistance[2:len(dataDistance) - 2]
        if valuedistance2 != b'None' and not valuedistance2.startswith(
                bytes('OK', 'UTF-8')):
...
def getSpeed(s: socket):
    ...
    if (dataSpeed.startswith(bytes('OK', 'UTF-8'))
            and dataSpeed.endswith(bytes('\r\n', 'UTF-8'))):
        valuespeed2 = dataSpeed[2:len(dataSpeed) - 2]
        if valuespeed2 != b'None' and not valuespeed2.startswith(
                bytes('OK', 'UTF-8')):
...
def Freerun():
    ...
    while True:
        ...
        #
        steering = Tk.Label(Freew,
                            text="Winkel Steuerung:",
                            fg='blue',
                            font=("Helvetica", 30))
        ...
        #
        attachment = Tk.Label(Freew,
                              text="Position Gerät:",
                              fg='blue',
                              font=("Helvetica", 30))
        attachment.place(x=900, y=300)
        attachement = Tk.Label(Freew,
                               text="Test",
                               fg='blue',
                               font=("Helvetica", 30))
        attachement.place(x=1300, y=300)
        #
        chassis = Tk.Label(Freew,
                           text="Position Chassis:",
                           fg='blue',
                           font=("Helvetica", 30))
        ...
        quit = Tk.Button(
            ...
            fg='blue',
            font=("Helvetica", 40),
            command=lambda: [stop(), Freew.destroy(),
                             s.close()])
        ...
        keyboard.on_press_key(
            "w", lambda _: sendToSpike(s, "AccelerationB.start(-100)"))
        keyboard.on_release_key(
            "w", lambda _: sendToSpike(s, "AccelerationB.stop()"))
        #backwards
        keyboard.on_press_key(
            "s", lambda _: sendToSpike(s, "AccelerationB.start(75)"))
        keyboard.on_release_key(
            "s", lambda _: sendToSpike(s, "AccelerationB.stop()"))
        #left
        keyboard.on_press_key("a",
                              lambda _: sendToSpike(s, "SteeringE.start(15)"))
        keyboard.on_release_key("a",
                                lambda _: sendToSpike(s, "SteeringE.stop()"))
        #right
        keyboard.on_press_key("d",
                              lambda _: sendToSpike(s, "SteeringE.start(-15)"))
        keyboard.on_release_key("d",
                                lambda _: sendToSpike(s, "SteeringE.stop()"))
        #chassis up and down
        keyboard.on_press_key(
            "y", lambda _: sendToSpike(s, "Chassis.run_for_rotations(5)"))
        keyboard.on_press_key(
            "x", lambda _: sendToSpike(s, "Chassis.run_for_rotations(-5)"))
        #Attachment up and down
        keyboard.on_press_key(
            "f", lambda _: sendToSpike(s, "Attachment.run_for_rotations(2)"))
        keyboard.on_press_key(
            "r", lambda _: sendToSpike(s, "Attachment.run_for_rotations(-2)"))
...
def updatecanvas():
    ...
    while True:
        ...
        if Geschw2 != 0:
            coordinatesX.append(positionX + Geschw2 / mapSize *
                                math.cos(math.radians(rotation)))
            coordinatesY.append(positionY + Geschw2 / mapSize *
                                math.sin(math.radians(rotation)))

        if Abstand2 != 0:
            obstacleX = (positionX + offsetX +
                         (math.cos(math.radians(rotation)) * Abstand2 * 0.75 /
                          mapSize * 20))
            obstacleY = (positionY + offsetY +
                         (math.sin(math.radians(rotation)) * Abstand2 * 0.75 /
                          mapSize * 20))
            obstaclesX.append(obstacleX)
            obstaclesY.append(obstacleY)
            canvas.create_rectangle(obstacleX + 5,
                                    obstacleY + 5,
                                    obstacleX - 5,
                                    obstacleY - 5,
                                    outline='black',
        ...
        if positionX + offsetX < 800 and positionY + offsetY < 800 and positionX + offsetX > 50 and positionY + offsetY > 50:
            ...
            rotated_img = resized_img.rotate(rotation * -1 + 180,
        ...
        if keyboard.is_pressed("+"):
            arrow.place(x=positionX * 1.2 - 10 + offsetX,
                        y=positionY * 1.2 - 10 + offsetY)
            updateMap(1.2)

            mapSize /= 1.2

        if keyboard.is_pressed("-"):
            arrow.place(x=positionX * 0.8 - 10 + offsetX,
                        y=positionY * 0.8 - 10 + offsetY)
            updateMap(0.8)
            mapSize /= 0.8
...
def updateMap(zoomvalue):
    ...
    for i in range(0, len(coordinatesX)):
        ...
        if i != 0:
            if coordinatesX[i] + offsetX < 800 and coordinatesY[
                    i] + offsetY < 800 and coordinatesX[
    ...
    for i in range(0, len(obstaclesX)):
        ...
        if obstaclesX[i] + offsetX < 800 and obstaclesY[
                i] + offsetY < 800 and obstaclesX[
                    i] + offsetX > 50 and obstaclesY[i] + offsetY > 50:
            canvas.create_rectangle(obstaclesX[i] + 5 + offsetX,
                                    obstaclesY[i] + 5 + offsetY,
                                    obstaclesX[i] - 5 + offsetX,
                                    obstaclesY[i] - 5 + offsetY,
                                    fill="black")

    #Resized_Line = Line.resize((mapSize, 20))
    #line_img = ImageTk.PhotoImage(Resized_Line, master=canvas)
    #line = Tk.Label(image = line_img)
    #line.config(image=line_img)
    #line.image = line_img